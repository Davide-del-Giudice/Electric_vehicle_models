...
Ac_ctrl control begin
	...
	Tr0  tran nettype=1 sparse=3 tstop=1 uic=1 acntrl=3 iabstol=1u
	Dc1    dc nettype=1 sparse=3 restart=no time=0
	Tr1  tran nettype=1 sparse=2 tstop=TSTOP acntrl=3 tmax=0.1m ;iabstol=1u
	
	TIME   = get("Tr1.time");
	P_EV   = get("Tr1.EV.p_ev");
	P_CS   = get("Tr1.EV.p_cs");
	TAP    = get("Tr1.tap");
	VD_PCC = get("Tr1.EV.v0d_pan");
	VQ_PCC = get("Tr1.EV.v0q_pan");
	ID_PCC = get("Tr1.EV.i0d_pan");
	IQ_PCC = get("Tr1.EV.i0q_pan");

#ifdef CCCV
save("mat5","DYN_CCCV_TRAN_KI_1000.mat","TIME",TIME,"P_EV",P_EV,"P_CS",P_CS,"TAP",TAP,"VD_PCC",VD_PCC,"VQ_PCC",VQ_PCC,"ID_PCC",ID_PCC,"IQ_PCC",IQ_PCC);
#else
save("mat5","DYN_CPCV_TRAN_KI_1000.mat","TIME",TIME,"P_EV",P_EV,"P_CS",P_CS,"TAP",TAP,"VD_PCC",VD_PCC,"VQ_PCC",VQ_PCC,"ID_PCC",ID_PCC,"IQ_PCC",IQ_PCC);
#endif

endcontrol 
#else

	Dc0    dc nettype=1 sparse=3 ;vreltol=1n ireltol=1n vabstol=1n
	Trb  tran nettype=1 sparse=2 tstop=TSTOP acntrl=3 tmax=0.1m iabstol=1u
...
TAP_DC tap_dc  gnd   vsource v1=1 v2=1.03 td=1+1u tr=1p \
                         width=TSTOP+1 period=TSTOP+2

TAP_AC tap_ac gnd vsource vdc=0 mag=0
