...
parameters LAMBDA        = 25.8/100;
;parameters LAMBDA        = 0/100;

; SWEEPING PARAMETERS
parameters START_LAMBDA  = 0/100;
parameters END_LAMBDA    = 30/100;
parameters STEP_LAMBDA   = 0.2/100;
...
;Trx  tran nettype=1 sparse=3 tstop=25 uic=1 acntrl=3 tmin=1m iabstol=1u
;Dc1    dc nettype=1 sparse=3 restart=no time=0 print=yes fixsm=yes load="overload_dyn.dc"
Dc1    dc nettype=1 sparse=3 time=0 print=yes fixsm=yes iabstol=1u load="overload_dyn.dc"
Pz     pz nettype=1 pf=1 shift=8.030027e-01 maxm=1e4
Tr1000  tran nettype=1 sparse=3 tstop=1000 acntrl=3 iabstol=1u tmax=10m
#else
include "save_dynamic.txt"
#endif
